...
Stable releases after v0.1.0 publish `jobman-diagnose` as signed Debian, RPM,
...
Each stable release after v0.1.0 also exposes the packages directly on GitHub:
...
the archive bytes to the tagged source. Releases after v0.1.0 additionally
include one SPDX SBOM per native Linux package. These records do not make an
untrusted build or model runtime safe.
