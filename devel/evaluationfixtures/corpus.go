...
func generatedCases() []evaluation.Case {
	cases := []evaluation.Case{
		...
		requiredFailure("python_child_process_exit", "python-11-child-process-exit-v1.json", "generated.application_configuration",
			...
			facts(fact("schema mismatch", "schema is version 8", "expected version 11"), fact("remediation range", "migrations 009 through 011")),
			relations(relation("schema mismatch caused child exit", []string{"schema is version 8", "migration rejected"}, []string{"exit status 17", "CalledProcessError"}))),
		...
		requiredFailure("python_pipeline_cause_chain", "python-18-pipeline-cause-chain-v1.json", "generated.data_validation",
			...
			relations(relation("decimal parse caused transform failure", []string{"InvalidOperation", "ConversionSyntax"}, []string{"RecordTransformError", "invalid decimal amount"}))),
		...
		requiredFailure("shell_pipeline_command", "shell-pipeline-command-v1.json", "generated.dependency_missing",
			[]string{"failure.dependency", "language.shell"}, facts(fact("command", "report-converter"), fact("output", "summary.pdf")), nil),
		...
		requiredFailure("node_service_cause", "node-service-cause-v1.json", "generated.external_service_failure",
			...
			relations(relation("service response caused checkout failure", []string{"HTTP 503", "Service Unavailable"}, []string{"checkout request failed"}))),
